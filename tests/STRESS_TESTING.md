...
Eight reduced fixtures guard the repaired behaviors:
...
All 22 CTest entries pass in Release, macOS ASan and UBSan, and Linux/glibc
UBSan builds. Rust passes 21 integration tests and three doc tests, with no
...
the SystemVerilog case is about 2× slower with the corrected conflict machinery.
...
These are fixture microbenchmarks, not whole-project measurements. The Go and
Solidity regressions are material; the corrected implementation is not claimed
to preserve conflict-heavy throughput. Profiling SystemVerilog found substantial
repeated lexing and speculative work: the simple assignment example explores
three distinct lexer modes at the same source position. An experiment preventing
inlining of the speculative driver changed SystemVerilog by roughly −1%, Go by
+1%, and Solidity by +3%; it was not retained. No parse-selection or token-reuse
rules were weakened for speed. Correctness takes priority over these documented
performance costs.
