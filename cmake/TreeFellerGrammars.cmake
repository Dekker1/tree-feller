...
# Download `url` to `dest`, checked against `sha256`; fatal on failure.
function(tf__download url dest sha256)
...
# Appends the parser to `tf_grammar_sources` in the caller's scope.
function(tf_fetch_crate_grammar name version sha256)
  set(parser "${TF_GRAMMAR_CACHE}/${name}-${version}/src/parser.c")
  if(NOT EXISTS "${parser}")
    set(archive "${TF_GRAMMAR_CACHE}/${name}-${version}.tar.gz")
    message(STATUS "tree-feller: fetching ${name} ${version}")
    tf__download("https://static.crates.io/crates/${name}/${name}-${version}.crate"
      "${archive}" ${sha256})
  ...
  set(tf_grammar_sources ${tf_grammar_sources} "${parser}" PARENT_SCOPE)
endfunction()
...
function(tf_add_grammars)
  ...
  tf_fetch_crate_grammar(tree-sitter-regex 0.25.0
    bd8a59be9f0ac131fd8f062eaaba14882b2fa5a6a7882a20134cb1d60df2e625)
  tf_fetch_raw_grammar(datazinc "${SHACKLE}/tree-sitter-datazinc/src/parser.c"
    070ea374153f806433b42a43b82fd976a243534e189b5d4dd8126c7b8b0e209a)
  tf_fetch_raw_grammar(minizinc "${SHACKLE}/tree-sitter-minizinc/src/parser.c"
    b2383631766367c40f63aff26ef62c0c18d38dec673246dfe749ec5662caf495)
  tf_fetch_raw_grammar(eprime "${SHACKLE}/tree-sitter-eprime/src/parser.c"
    eabd6e18fb9feb50b258bc5589012d795713fe4ef860a53297b39bbd991298d4)
  ...
  add_library(tf_grammars STATIC ${tf_grammar_sources})
